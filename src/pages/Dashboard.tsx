...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const [profileResult, countResult] = await Promise.all([
        supabase
          .from('profiles')
          .select('exam_history, used_questions, display_name, avatar_url, email')
          .eq('id', user.id)
          .maybeSingle(),
        supabase
          .from('questions')
  ...
  const overallProgress =
    totalQuestionCount > 0
      ? Math.round((usedQuestions.length / totalQuestionCount) * 100)
      : 0;

  const last3 = [...examHistory].reverse().slice(0, 3);
  const userName = displayName || email || 'Utilisateur';
        ...
        <div className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <Card>
            ...
            <CardContent className="flex items-center justify-center pb-6">
              <CircularProgress value={overallProgress} label={`${usedQuestions.length} / ${totalQuestionCount} questions`} />
            </CardContent>
          </Card>
            ...
            <CardContent className="flex flex-col items-center justify-center gap-3 pb-6">
              {avgScore >= 70 ? (
                <>
                  ...
                  <span className="text-lg font-semibold text-foreground">On Track</span>
                  <span className="text-sm text-muted-foreground">Score moyen : {avgScore}%</span>
                </>
              ) : (
                <>
                  ...
                  <span className="text-lg font-semibold text-foreground">Needs Practice</span>
                  <span className="text-sm text-muted-foreground">Score moyen : {avgScore}%</span>
                </>
              )}
            </CardContent>
          ...
          <Card className="relative overflow-hidden">
            ...
            <CardContent className="pb-6">
              {last3.length === 0 ? (
                <p className="py-4 text-center text-sm text-muted-foreground">Aucun examen passé</p>
              ) : (
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between rounded-md border border-border/30 bg-secondary/30 px-3 py-2">
                        <span className="text-sm text-foreground">
                          Quiz #{examHistory.length - i}
                        </span>
                        <Badge variant={entry.passed ? 'default' : 'secondary'} className="text-xs">
                          {percent}%
                        </Badge>
                      </div>
                    );
            ...
            {tier === 'free' && (
              <div className="absolute inset-0 flex items-center justify-center rounded-lg bg-background/80 backdrop-blur-md">
                <div className="text-center">
                  ...
                  <p className="mb-3 text-sm font-medium text-foreground">Réservé aux abonnés</p>
        ...
        <div className="glass-card mb-8 p-6">
          ...
          <div className="relative">
            <p className="mb-4 text-sm text-muted-foreground">
              ...
              {!isTier1OrAbove && ' (réservé aux abonnés Essential+)'}
            </p>
